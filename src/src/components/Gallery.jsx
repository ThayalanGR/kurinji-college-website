...
export default class Gallery extends Component {
  ...
  firePreviewContainer(src) {
    toast(
      <img className="gallery-preview-container-image img img-responsive" src={src} alt="" srcSet="" />, {
        position: "top-left",
          ...
          <div className="row">
            {this.state.galleryItems.map((item, key) => (
              <div className="col-sm-12 col-md-2 text-center mb-2" key={key}>
                <img onClick={(e) => { this.firePreviewContainer(e.target.src) }} className="gallery-image-thumbnail hoverable" src={item[1]} alt="" srcSet="" />
              </div>
            ))}
          </div>
        </div>
